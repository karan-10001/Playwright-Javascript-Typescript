...
test.beforeAll(async ({ browser }) => {
  ...
  await userEmailLocator.fill("karansocialmedia10001@gmail.com");
  const userPasswordLocator = page.locator("#userPassword");
  await userPasswordLocator.fill("Singh12345");
  ...
  //here you are getting the state.json file that have all parameter
  await context.storageState({ path: "state.json" });

  // now you are injecting the json to brower context & storing it's context as webcontext
  // that you  will for every test to bypass login\
  webContext = await browser.newContext({ storageState: "state.json" });
});

test("Place the roder in shop E2E test case", async () => {
