...
test("extract email from text in child window test", async ({ browser }) => {
  const browserContext = await browser.newContext();
  const page = await browserContext.newPage();
...
test("Get the email from child window & use this on parent window test", async ({
  browser,
}) => {
  const browserContext = await browser.newContext();
  const page = await browserContext.newPage();
  ...
  console.log(
    "content of user input field in parent window : ",
